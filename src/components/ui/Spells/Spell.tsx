...
export const Spell: React.FC<SpellProps> = ({
    ...
    const { isPrinting } = useCharacter();
    const color = isPrinting
        ? '#000'
        : magicSchoolsByColor[spell.school as keyof typeof magicSchoolsByColor];
    return (
        <div className='pt-[3mm]'>
            <div
                key={spell.id}
                className={cx(
                    styles.spell,
                    'flex flex-col border-2 rounded p-2 font-Roboto font-normal break-inside-avoid',
                    {
                        'bg-white': selected,
            ...
            >
                ...
                >
                    <h1
                        className={cx('font-Nunito', {
                            'text-[4.5mm]': minimized,
                        })}
                    >
                ...
                {!minimized && (
                    <>
                        ...
                        <div>
                            {spell.description.map(
                                (descriptionPiece, index) => (
                                    <p
